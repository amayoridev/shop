...
app.post('/api/orders', async (req, res) => {
  try {
    const { items, tableN, payment, status } = req.body;
...
app.post('/api/ordersAPI', async (req, res) => {
  const { id } = req.body;
    try {
      const result = await Orders.findOneAndUpdate({ _id: id }, {payment: true});
      
      if (result.deletedCount > 0) {
        res.json({ message: "Order deleted" })
      } else {
        res.json({ message: `${id} not found on DB` });
      }
    } catch (err) {
      console.error('Error deleting document:', err);
      res.render('index', { alertMessage: 'Error occurred while deleting' });
